	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  md_addr_t ps_tag = CACHE_TAG_PSEUDOASSOC(cp,addr);
  md_addr_t blk_ps_tag;
  md_addr_t bit_flip;
  ...
  if((strcmp(cp->name,"dl1")==0)  && (use_colassoc == TRUE)){
    //printf("****L1 Cache****\n"); Column Assoc Cache

    // Check if b[a] is a hit
    blk=cp->sets[set].way_head;
    if((blk!= NULL) &&(blk->tag == ps_tag) && (blk->status & CACHE_BLK_VALID))
    {
      goto cache_hit;
    }
    // Check if r-bit is 1
    if(blk->rehash_bit == 1){
      // Clobber1
      // Miss get data and place in blk
      // Reset the rehash bit
      blk->rehash_bit = 0;
    }
    else{
      // f[a] is hit/miss
      bit_flip = BIT_FLIP(cp,set);
      alt_blk=cp->sets[bit_flip].way_head;
      if((alt_blk!= NULL) &&(alt_blk->tag == ps_tag) && (alt_blk->status & CACHE_BLK_VALID))
      {
        // f[a] is hit, swap and done
        swap(cp,set,bit_flip);
        goto cache_hit;
      }
      else{

        //Miss, get data and place in f[a]
        // set the rehash bit to 1
        alt_blk->rehash_bit = 1;
        set = bit_flip;
